...
RCT_EXPORT_VIEW_PROPERTY(onYatzyGridDetected, RCTDirectEventBlock);
...
- (NSDictionary *)constantsToExport {
  return @{
    @"Type" : @{@"front" : @(RNCameraTypeFront), @"back" : @(RNCameraTypeBack)},
    @"FlashMode" : @{
...
- (NSArray<NSString *> *)supportedEvents {
  return @[
    @"onCameraReady", @"onAudioInterrupted", @"onAudioConnected",
    @"onMountError", @"onBarCodeRead", @"onFacesDetected", @"onPictureTaken",
    @"onPictureSaved", @"onRecordingStart", @"onRecordingEnd",
    @"onTextRecognized", @"onYatzyGridDetected",
    @"onGoogleVisionBarcodesDetected", @"onSubjectAreaChanged"
  ];
}
...
+ (NSDictionary *)validCodecTypes {
  if (@available(iOS 11, *)) {
    ...
    return @{@"H264" : AVVideoCodecH264, @"JPEG" : AVVideoCodecJPEG};
  }
}
...
RCT_CUSTOM_VIEW_PROPERTY(yatzyGridDetectorEnabled, BOOL, RNCamera) {

  view.canDetectYatzyGrid = [RCTConvert BOOL:json];
  [view setupOrDisableYatzyGridDetector];
}
...
RCT_CUSTOM_VIEW_PROPERTY(defaultVideoQuality, NSInteger, RNCamera) {
  [view
      setDefaultVideoQuality:[NSNumber numberWithInteger:[RCTConvert
                                                             NSInteger:json]]];
}

RCT_REMAP_METHOD(takePicture, options
                 : (NSDictionary *)options reactTag
                 : (nonnull NSNumber *)reactTag resolver
                 : (RCTPromiseResolveBlock)resolve rejecter
                 : (RCTPromiseRejectBlock)reject) {
  [self.bridge.uiManager addUIBlock:^(__unused RCTUIManager *uiManager,
                                      NSDictionary<NSNumber *, RNCamera *>
                                          *viewRegistry) {
    RNCamera *view = viewRegistry[reactTag];
    if (![view isKindOfClass:[RNCamera class]]) {
      RCTLogError(
          @"Invalid view returned from registry, expecting RNCamera, got: %@",
          view);
    } else {
      ...
      UIImage *generatedPhoto =
          [RNImageUtils generatePhotoOfSize:CGSizeMake(200, 200)];
      BOOL useFastMode =
          options[@"fastMode"] && [options[@"fastMode"] boolValue];
      if (useFastMode) {
        resolve(nil);
      }
...
RCT_REMAP_METHOD(record, withOptions
                 : (NSDictionary *)options reactTag
                 : (nonnull NSNumber *)reactTag resolver
                 : (RCTPromiseResolveBlock)resolve rejecter
                 : (RCTPromiseRejectBlock)reject) {
#if TARGET_IPHONE_SIMULATOR
  reject(@"E_RECORDING_FAILED",
         @"Video recording is not supported on a simulator.", nil);
  return;
#endif
  [self.bridge.uiManager addUIBlock:^(__unused RCTUIManager *uiManager,
                                      NSDictionary<NSNumber *, RNCamera *>
...
RCT_EXPORT_METHOD(checkDeviceAuthorizationStatus
...
RCT_EXPORT_METHOD(checkVideoAuthorizationStatus
                  : (RCTPromiseResolveBlock)resolve reject
                  : (__unused RCTPromiseRejectBlock)reject) {
#ifdef DEBUG
  if ([[NSBundle mainBundle].infoDictionary
          objectForKey:@"NSCameraUsageDescription"] == nil) {
    RCTLogWarn(
        @"Checking video permissions without having key "
        @"'NSCameraUsageDescription' defined in your Info.plist. If you do not "
        @"add it your app will crash when being built in release mode. You "
        @"will have to add it to your Info.plist file, otherwise RNCamera is "
        @"not allowed to use the camera.  You can learn more about adding "
        @"permissions here: https://stackoverflow.com/a/38498347/4202031");
    resolve(@(NO));
    return;
  }
#endif
...
RCT_EXPORT_METHOD(checkRecordAudioAuthorizationStatus
                  : (RCTPromiseResolveBlock)resolve reject
                  : (__unused RCTPromiseRejectBlock)reject) {
  if ([[NSBundle mainBundle].infoDictionary
          objectForKey:@"NSMicrophoneUsageDescription"] == nil) {
    RCTLogWarn(@"Checking audio permissions without having key "
               @"'NSMicrophoneUsageDescription' defined in your Info.plist. "
               @"Audio Recording for your video files is therefore disabled. "
               @"If you do not need audio on your recordings is is recommended "
               @"to set the 'captureAudio' property on your component instance "
               @"to 'false', otherwise you will have to add the key "
               @"'NSMicrophoneUsageDescription' to your Info.plist. If you do "
               @"not your app will crash when being built in release mode. You "
               @"can learn more about adding permissions here: "
               @"https://stackoverflow.com/a/38498347/4202031");
    resolve(@(NO));
    return;
  } else {
#ifdef DEBUG
...
RCT_REMAP_METHOD(getAvailablePictureSizes, ratio
                 : (NSString *)ratio reactTag
                 : (nonnull NSNumber *)reactTag resolver
                 : (RCTPromiseResolveBlock)resolve rejecter
                 : (RCTPromiseRejectBlock)reject) {
...
RCT_EXPORT_METHOD(isRecording
                  : (nonnull NSNumber *)reactTag resolver
                  : (RCTPromiseResolveBlock)resolve rejecter
                  : (RCTPromiseRejectBlock)reject) {
#if TARGET_IPHONE_SIMULATOR
  reject(@"E_IS_RECORDING_FAILED",
         @"Video recording is not supported on a simulator.", nil);
  return;
#endif
  [self.bridge.uiManager addUIBlock:^(__unused RCTUIManager *uiManager,
...
RCT_EXPORT_METHOD(getCameraIds
                  : (RCTPromiseResolveBlock)resolve reject
                  : (RCTPromiseRejectBlock)reject) {
