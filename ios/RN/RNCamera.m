...
@property(nonatomic, strong) id yatzyGridDetector;
...
@property(nonatomic, copy) RCTDirectEventBlock onYatzyGridDetected;
...
@property(nonatomic, assign) BOOL finishedDetectingYatzyGrid;
...
@property(nonatomic, copy) NSDate *startYatzyGrid;
...
- (id)initWithBridge:(RCTBridge *)bridge {
  if ((self = [super init])) {
    ...
    self.yatzyGridDetector = [self createYatzyGridDetector];
...
- (void)onYatzyGrid:(NSDictionary *)event {
  if (_onYatzyGridDetected && _session) {
    _onYatzyGridDetected(event);
  }
}
...
- (void)insertReactSubview:(UIView *)view atIndex:(NSInteger)atIndex {
  [self insertSubview:view
              atIndex:atIndex + 1]; // is this + 1 really necessary?
...
- (AVCaptureDevice *)getDevice {
  ...
  if (self.cameraId != nil) {
    ...
    captureDevice = [RNCameraUtils deviceWithMediaType:AVMediaTypeVideo
...
- (AVCaptureSessionPreset)getDefaultPreset {
  AVCaptureSessionPreset preset =
      ([self pictureSize] && [[self pictureSize] integerValue] >= 0)
          ? [self pictureSize]
...
- (AVCaptureSessionPreset)getDefaultPresetVideo {
  ...
  AVCaptureSessionPreset preset =
      ([self defaultVideoQuality])
          ? [RNCameraUtils
                captureSessionPresetForVideoResolution:[[self
                                                           defaultVideoQuality]
                                                           integerValue]]
...
- (void)defocusPointOfInterest {
  ...
  if (self.isFocusedOnPoint) {
    ...
    [self onSubjectAreaChanged:@{
      @"prevPointOfInterest" : @{@"x" : @(prevPoint.x), @"y" : @(prevPoint.y)}
    }];
  }
...
- (void)updateAutoFocusPointOfInterest {
  ...
  if ([self.autoFocusPointOfInterest objectForKey:@"x"] &&
      [self.autoFocusPointOfInterest objectForKey:@"y"]) {
    ...
    if ([self.autoFocusPointOfInterest objectForKey:@"autoExposure"]) {
      BOOL autoExposure =
          [self.autoFocusPointOfInterest[@"autoExposure"] boolValue];
...
- (void)updateFocusDepth {
  ...
  if (device == nil || self.autoFocus < 0 ||
      device.focusMode != RNCameraAutoFocusOff ||
  ...
  [device setFocusModeLockedWithLensPosition:self.focusDepth
...
- (void)updateWhiteBalance {
  ...
  if (self.whiteBalance == RNCameraWhiteBalanceAuto) {
    ...
    AVCaptureWhiteBalanceTemperatureAndTintValues temperatureAndTint = {
        .temperature =
            [RNCameraUtils temperatureForWhiteBalance:self.whiteBalance],
        .tint = 0,
    };
    AVCaptureWhiteBalanceGains rgbGains = [device
        deviceWhiteBalanceGainsForTemperatureAndTintValues:temperatureAndTint];
    __weak __typeof__(device) weakDevice = device;
    if ([device lockForConfiguration:&error]) {
      @try {
        [device
            setWhiteBalanceModeLockedWithDeviceWhiteBalanceGains:rgbGains
                                               completionHandler:^(
                                                   CMTime syncTime) {
                                                 [weakDevice
...
/// which is a float between 0 and 1 if defined by the user or -1 to indicate
/// that no selection is active. 'exposure' gets mapped to a valid ISO value
/// between the device's min/max-range of ISO-values.
///
/// The exposure gets reset every time the user manually sets the
/// autofocus-point in 'updateAutoFocusPointOfInterest' automatically. Currently
/// no explicit event is fired. This leads to two 'exposure'-states: one here
/// and one in the component, which is fine. 'exposure' here gets only synced if
/// 'exposure' on the js-side changes. You can manually keep the state in sync
/// by setting 'exposure' in your React-state everytime the
/// js-updateAutoFocusPointOfInterest-function gets called.
- (void)updateExposure {
  AVCaptureDevice *device = [self.videoCaptureDeviceInput device];
  NSError *error = nil;
  ...
  // Lazy init of range.
  if (!self.exposureIsoMin) {
    self.exposureIsoMin = device.activeFormat.minISO;
  }
  if (!self.exposureIsoMax) {
    self.exposureIsoMax = device.activeFormat.maxISO;
  }

  // Get a valid ISO-value in range from min to max. After we mapped the
  // exposure (a val between 0 - 1), the result gets corrected by the offset
  // from 0, which is the min-ISO-value.
  float appliedExposure =
      (self.exposureIsoMax - self.exposureIsoMin) * self.exposure +
      self.exposureIsoMin;

  // Make sure we're in AVCaptureExposureModeCustom, else the ISO + duration
  // time won't apply. Also make sure the device can set exposure
  if ([device isExposureModeSupported:AVCaptureExposureModeCustom]) {
    if (device.exposureMode != AVCaptureExposureModeCustom) {
      [device setExposureMode:AVCaptureExposureModeCustom];
    }

    // Only set the ISO for now, duration will be default as a change might
    // affect frame rate.
...
- (void)takePictureWithOrientation:(NSDictionary *)options
  ...
  [self.sensorOrientationChecker
      getDeviceOrientationWithBlock:^(UIInterfaceOrientation orientation) {
        NSMutableDictionary *tmpOptions = [options mutableCopy];
        if ([tmpOptions valueForKey:@"orientation"] == nil) {
          tmpOptions[@"orientation"] = [NSNumber
              numberWithInteger:
                  [self.sensorOrientationChecker
                      convertToAVCaptureVideoOrientation:orientation]];
        }
        self.deviceOrientation = [NSNumber numberWithInteger:orientation];
        self.orientation = [NSNumber
            numberWithInteger:[tmpOptions[@"orientation"] integerValue]];
...
- (void)takePicture:(NSDictionary *)options
  ...
  @try {
    [self.stillImageOutput captureStillImageAsynchronouslyFromConnection:
                               connection
                                                       completionHandler:
                                                           ^(CMSampleBufferRef imageSampleBuffer, NSError
                                                                                                      *error) {
                                                             if (imageSampleBuffer &&
                                                               ...
                                                               // metadata idea
                                                               // taken from:
                                                               // https://stackoverflow.com/questions/9006759/how-to-write-exif-metadata-to-an-image-not-the-camera-roll-just-a-uiimage-or-j/9091472
                                                               NSMutableData
                                                                   *destData =
                                                                       [NSMutableData
                                                                           data];
                                                               ...
                                                               if (CGImageDestinationFinalize(
                                                                 ...
                                                                 if ([options[@"exif"]
                                                                   ...
                                                                   // now
                                                                   //[RNImageUtils
                                                                   // updatePhotoMetadata:imageSampleBuffer
                                                                   // withAdditionalData:@{
                                                                   //@"Orientation":
                                                                   //@(imageRotation)
                                                                   //}
    ...
    reject(
        @"E_IMAGE_CAPTURE_FAILED", @"Got exception while taking picture",
        [NSError
            errorWithDomain:@"E_IMAGE_CAPTURE_FAILED"
                       code:500
...
- (void)record:(NSDictionary *)options
  ...
  if (_movieFileOutput == nil) {
    // At the time of writing AVCaptureMovieFileOutput and
    // AVCaptureVideoDataOutput (> GMVDataOutput) cannot coexist on the same
    // AVSession (see: https://stackoverflow.com/a/4986032/1123156). We stop
    // face detection here and restart it in when AVCaptureMovieFileOutput
    // finishes recording.
    if ([self.yatzyGridDetector isRealDetector]) {
      [self stopYatzyGridDetection];
    }
  ...
  if (self.movieFileOutput == nil || self.movieFileOutput.isRecording ||
      _videoRecordedResolve != nil || _videoRecordedReject != nil) {
  ...
  if (options[@"quality"]) {
    AVCaptureSessionPreset newQuality = [RNCameraUtils
        captureSessionPresetForVideoResolution:
            (RNCameraVideoResolution)[options[@"quality"] integerValue]];
  ...
  if (self.videoStabilizationMode != 0) {
    if (connection.isVideoStabilizationSupported == NO) {
      RCTLogWarn(@"%s: Video Stabilization is not supported on this device.",
                 __func__);
    } else {
      [connection
          setPreferredVideoStabilizationMode:self.videoStabilizationMode];
    }
  }
  [connection setVideoOrientation:orientation];

  BOOL recordAudio =
      [options valueForKey:@"mute"] == nil ||
      ([options valueForKey:@"mute"] != nil && ![options[@"mute"] boolValue]);

  // sound recording connection, we can easily turn it on/off without
  // manipulating inputs, this prevents flickering. note that mute will also be
  // set to true if captureAudio is set to false on the JS side. Check the
  // property anyways just in case it is manipulated with setNativeProps
  if (recordAudio && self.captureAudio) {
    ...
    if (self.audioCaptureDeviceInput != nil) {
      AVCaptureConnection *audioConnection =
          [self.movieFileOutput connectionWithMediaType:AVMediaTypeAudio];
  ...
  dispatch_async(self.sessionQueue, ^{
    ...
    if (options[@"maxDuration"]) {
      ...
      self.movieFileOutput.maxRecordedDuration =
          CMTimeMakeWithSeconds(maxDuration, 30);
    }

    if (options[@"maxFileSize"]) {
      self.movieFileOutput.maxRecordedFileSize =
          [options[@"maxFileSize"] integerValue];
    }

    if (options[@"codec"]) {
      if (@available(iOS 10, *)) {
        AVVideoCodecType videoCodecType = options[@"codec"];
        if ([self.movieFileOutput.availableVideoCodecTypes
                containsObject:videoCodecType]) {
          ...
          if (options[@"videoBitrate"]) {
            ...
            [self.movieFileOutput
                setOutputSettings:@{AVVideoCodecKey : videoCodecType}
                    forConnection:connection];
          }
        } else {
        ...
        RCTLogWarn(
            @"%s: Setting videoCodec is only supported above iOS version 10.",
            __func__);
      }
    }
    ...
    if (options[@"path"]) {
      ...
      path = [RNFileSystem
          generatePathInDirectory:[[RNFileSystem cacheDirectoryPath]
    ...
    dispatch_after(popTime, self.sessionQueue, ^(void) {
      ...
      if (self.movieFileOutput != nil && self.videoCaptureDeviceInput != nil &&
          _recordRequested) {
        ...
        reject(@"E_VIDEO_CAPTURE_FAILED",
               !_recordRequested ? @"Recording request cancelled."
                                 : @"Camera is not ready.",
               nil);
        [self cleanupCamera];
      }
...
- (void)startSession {
  ...
  dispatch_async(self.sessionQueue, ^{
    ...
    // (see comment in -record), we go ahead and add the
    // AVCaptureMovieFileOutput to avoid an exposure rack on some devices that
    // can cause the first few frames of the recorded output to be underexposed.
    if (![self.faceDetector isRealDetector] &&
        ![self.textDetector isRealDetector] &&
        ![self.yatzyGridDetector isRealDetector] &&
...
- (void)initializeAudioCaptureSessionInput {
  ...
  if (self.audioCaptureDeviceInput == nil) {
    ...
    AVCaptureDeviceInput *audioDeviceInput =
        [AVCaptureDeviceInput deviceInputWithDevice:audioCaptureDevice
                                              error:&error];
    ...
    if (error || audioDeviceInput == nil) {
      ...
      BOOL setActive = [[AVAudioSession sharedInstance] setActive:YES
                                                            error:&error];

      if (!setActive) {
        RCTLogWarn(@"Audio device could not set active: %s: %@", __func__,
                   error);
      }
...
- (void)removeAudioCaptureSessionInput {
  if (self.audioCaptureDeviceInput != nil) {
    ...
    if (!self.keepAudioSession) {
      ...
      BOOL setInactive = [[AVAudioSession sharedInstance]
            setActive:NO
          withOptions:AVAudioSessionSetActiveOptionNotifyOthersOnDeactivation
                error:&error];

      if (!setInactive) {
        RCTLogWarn(@"Audio device could not set inactive: %s: %@", __func__,
                   error);
      }
    }
...
- (void)initializeCaptureSessionInput {
  ...
  dispatch_async(self.sessionQueue, ^{
    ...
    // return.
    if (self.videoCaptureDeviceInput != nil && captureDevice != nil &&
        [self.videoCaptureDeviceInput.device.uniqueID
            isEqualToString:captureDevice.uniqueID]) {
      return;
    }
    ...
    AVCaptureDevice *previousDevice = self.videoCaptureDeviceInput != nil
    ...
    if ([self.session canAddInput:captureDeviceInput]) {
      ...
      RCTLog(@"The selected device does not work with the Preset [%@] or "
             @"configuration provided",
             self.session.sessionPreset);

      [self onMountingError:@{
        @"message" : @"Camera device does not support selected settings."
      }];
    }
...
- (void)updateSessionPreset:(AVCaptureSessionPreset)preset {
  ...
  if (preset) {
    if (self.canDetectFaces && [preset isEqual:AVCaptureSessionPresetPhoto]) {
      RCTLog(@"AVCaptureSessionPresetPhoto not supported during face "
             @"detection. Falling back to AVCaptureSessionPresetHigh");
    ...
    dispatch_async(self.sessionQueue, ^{
      if ([self.session canSetSessionPreset:preset]) {
        ...
        RCTLog(
            @"The selected preset [%@] does not work with the current session.",
            preset);
      }
    });
  }
...
- (void)audioDidInterrupted:(NSNotification *)notification {
  ...
  NSInteger type =
      [[userInfo valueForKey:AVAudioSessionInterruptionTypeKey] integerValue];
  ...
  if (type == AVAudioSessionInterruptionTypeEnded) {
    ...
    NSInteger option = [[userInfo
        valueForKey:AVAudioSessionInterruptionOptionKey] integerValue];
...
- (void)sessionWasInterrupted:(NSNotification *)notification {
  ...
  NSDictionary *userInfo = notification.userInfo;
  NSInteger type = [[userInfo valueForKey:AVCaptureSessionInterruptionReasonKey]
      integerValue];
...
- (void)changePreviewOrientation:(UIInterfaceOrientation)orientation {
  ...
  AVCaptureVideoOrientation videoOrientation =
      [RNCameraUtils videoOrientationForInterfaceOrientation:orientation];
  dispatch_async(dispatch_get_main_queue(), ^{
    __strong typeof(self) strongSelf = weakSelf;
    if (strongSelf &&
        strongSelf.previewLayer.connection.isVideoOrientationSupported) {
...
- (void)_setupOrDisableMetadataOutput {
  if ([self isReadingBarCodes] &&
      (_metadataOutput == nil ||
       ![self.session.outputs containsObject:_metadataOutput])) {
...
- (void)_updateMetadataObjectsToRecognize {
  ...
  NSArray<AVMetadataObjectType> *requestedObjectTypes =
      [NSArray arrayWithArray:self.barCodeTypes];
  NSArray<AVMetadataObjectType> *availableObjectTypes =
  ...
  for (AVMetadataObjectType objectType in requestedObjectTypes) {
    if ([availableObjectTypes containsObject:objectType]) {
      availableRequestedObjectTypes =
          [availableRequestedObjectTypes arrayByAddingObject:objectType];
    }
  }
          ...
          NSMutableDictionary *event =
              [NSMutableDictionary dictionaryWithDictionary:@{
                ...
                @"bounds" : @{
                  @"origin" : @{
                    @"x" : [NSString
                        stringWithFormat:@"%f", transformed.bounds.origin.x],
                    @"y" : [NSString
                        stringWithFormat:@"%f", transformed.bounds.origin.y]
                  },
                  @"size" : @{
                    @"height" : [NSString
                        stringWithFormat:@"%f", transformed.bounds.size.height],
                    @"width" : [NSString
                        stringWithFormat:@"%f", transformed.bounds.size.width]
                  }
                }
              }];

          NSData *rawData;
          // If we're on ios11 then we can use `descriptor` to access the raw
          // data of the barcode. If we're on an older version of iOS we're
          // stuck using valueForKeyPath to peak at the data.
          if (@available(iOS 11, *)) {
            // descriptor is a CIBarcodeDescriptor which is an abstract base
            // class with no useful fields. in practice it's a subclass, many of
            // which contain errorCorrectedPayload which is the data we want.
            // Instead of individually checking the class types, just duck type
            // errorCorrectedPayload
            if ([codeMetadata.descriptor
                    respondsToSelector:@selector(errorCorrectedPayload)]) {
              rawData = [codeMetadata.descriptor
                  performSelector:@selector(errorCorrectedPayload)];
            }
          } else {
            rawData = [codeMetadata
                valueForKeyPath:@"_internal.basicDescriptor.BarcodeRawData"];
          }

          // Now that we have the raw data of the barcode translate it into a
          // hex string to pass to the JS
          const unsigned char *dataBuffer =
              (const unsigned char *)[rawData bytes];
          if (dataBuffer) {
            NSMutableString *rawDataHexString =
                [NSMutableString stringWithCapacity:([rawData length] * 2)];
            ...
            [event setObject:[NSString stringWithString:rawDataHexString]
                      forKey:@"rawData"];
          }

          // If we were able to extract a string representation of the barcode,
          // attach it to the event as well else just send null along.
          if (codeMetadata.stringValue) {
            [event setObject:codeMetadata.stringValue forKey:@"data"];
          }

          // Only send the event if we were able to pull out a binary or string
          // representation
          if ([event objectForKey:@"data"] != [NSNull null] ||
              [event objectForKey:@"rawData"] != [NSNull null]) {
...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput
  ...
  if (success && self.videoRecordedResolve != nil) {
    ...
    if (@available(iOS 10, *)) {
      ...
      if (videoCodec == nil) {
        videoCodec =
            [self.movieFileOutput.availableVideoCodecTypes firstObject];
      }
      result[@"codec"] = videoCodec;

      if ([connections[0] isVideoMirrored]) {
        [self mirrorVideo:outputFileURL
               completion:^(NSURL *mirroredURL) {
    ...
    self.videoRecordedReject(@"E_RECORDING_FAILED",
...
- (void)cleanupCamera {
  ...
  if ([self.yatzyGridDetector isRealDetector]) {
    [self setupOrDisableYatzyGridDetector];
  }
...
- (void)mirrorVideo:(NSURL *)inputURL
         completion:(void (^)(NSURL *outputUR))completion {
  ...
  AVMutableVideoComposition *videoComposition =
      [[AVMutableVideoComposition alloc] init];
  videoComposition.renderSize = CGSizeMake(clipVideoTrack.naturalSize.height,
                                           clipVideoTrack.naturalSize.width);
  ...
  AVMutableVideoCompositionInstruction *instruction =
      [[AVMutableVideoCompositionInstruction alloc] init];
  instruction.timeRange =
      CMTimeRangeMake(kCMTimeZero, CMTimeMakeWithSeconds(60, 30));

  CGAffineTransform transform = CGAffineTransformMakeScale(-1.0, 1.0);
  transform = CGAffineTransformTranslate(transform,
                                         -clipVideoTrack.naturalSize.width, 0);
  ...
  AVAssetExportSession *exportSession =
      [AVAssetExportSession exportSessionWithAsset:videoAsset
                                        presetName:AVAssetExportPreset640x480];
  NSString *filePath = [RNFileSystem
      generatePathInDirectory:[[RNFileSystem cacheDirectoryPath]
                                  stringByAppendingString:@"CameraFlip"]
                withExtension:@".mp4"];
...
- (id)createFaceDetectorMlKit {
  Class faceDetectorManagerClassMlkit =
      NSClassFromString(@"FaceDetectorManagerMlkit");
...
- (void)stopFaceDetection {
  if (self.videoDataOutput && !self.canReadText && !self.canDetectYatzyGrid) {
...
- (void)updateFaceDetectionLandmarks:(id)requestedLandmarks {
  [self.faceDetector setLandmarksMode:requestedLandmarks
                                queue:self.sessionQueue];
}

- (void)updateFaceDetectionClassifications:(id)requestedClassifications {
  [self.faceDetector setClassificationMode:requestedClassifications
                                     queue:self.sessionQueue];
}
...
- (id)createBarcodeDetectorMlKit {
  Class barcodeDetectorManagerClassMlkit =
      NSClassFromString(@"BarcodeDetectorManagerMlkit");
...
#pragma mark - YatzyGridDetector

- (id)createYatzyGridDetector {
  Class yatzyGridDetectorManagerClass =
      NSClassFromString(@"YatzyGridDetectorManager");
  return [[yatzyGridDetectorManagerClass alloc] init];
}

- (void)setupOrDisableYatzyGridDetector {
  if ([self canDetectYatzyGrid] && [self.yatzyGridDetector isRealDetector]) {
    AVCaptureSessionPreset preset = [self getDefaultPresetVideo];
    self.session.sessionPreset = preset;
    if (!self.videoDataOutput) {
      self.videoDataOutput = [[AVCaptureVideoDataOutput alloc] init];
      if (![self.session canAddOutput:_videoDataOutput]) {
        NSLog(@"Failed to setup video data output for YatzyGrid detection");
        [self stopYatzyGridDetection];
        return;
      }
      NSDictionary *rgbOutputSettings = [NSDictionary
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput
  ...
  // 3. let user disable text recognition, e.g. onTextRecognized={someCondition
  // ? null : this.textRecognized}
  NSDate *methodFinish = [NSDate date];
  NSTimeInterval timePassedSinceSubmittingForText =
      [methodFinish timeIntervalSinceDate:self.startText];
  NSTimeInterval timePassedSinceSubmittingForYatzyGrid =
      [methodFinish timeIntervalSinceDate:self.startYatzyGrid];
  NSTimeInterval timePassedSinceSubmittingForFace =
      [methodFinish timeIntervalSinceDate:self.startFace];
  NSTimeInterval timePassedSinceSubmittingForBarcode =
      [methodFinish timeIntervalSinceDate:self.startBarcode];
  BOOL canSubmitForTextDetection = timePassedSinceSubmittingForText > 0.5 &&
                                   _finishedReadingText && self.canReadText &&
                                   [self.textDetector isRealDetector];
  BOOL canSubmitForYatzyGridDetection =
      timePassedSinceSubmittingForYatzyGrid > 0.5 &&
      _finishedDetectingYatzyGrid && self.canDetectYatzyGrid &&
      [self.yatzyGridDetector isRealDetector];
  BOOL canSubmitForFaceDetection =
      timePassedSinceSubmittingForFace > 0.5 && _finishedDetectingFace &&
      self.canDetectFaces && [self.faceDetector isRealDetector];
  BOOL canSubmitForBarcodeDetection =
      timePassedSinceSubmittingForBarcode > 0.5 && _finishedDetectingBarcodes &&
      self.canDetectBarcodes && [self.barcodeDetector isRealDetector];
  if (canSubmitForFaceDetection || canSubmitForTextDetection ||
      canSubmitForBarcodeDetection || canSubmitForYatzyGridDetection) {
    CGSize previewSize = CGSizeMake(_previewLayer.frame.size.width,
                                    _previewLayer.frame.size.height);
    NSInteger position = self.videoCaptureDeviceInput.device.position;
    UIImage *image = [RNCameraUtils convertBufferToUIImage:sampleBuffer
                                               previewSize:previewSize
                                                  position:position];
    // take care of the fact that preview dimensions differ from the ones of the
    // image that we submit for text detection
    ...
    if (canSubmitForTextDetection) {
      ...
      self.startText = [NSDate date];
      [self.textDetector findTextBlocksInFrame:image
                                        scaleX:scaleX
                                        scaleY:scaleY
                                     completed:^(NSArray *textBlocks) {
                                       NSDictionary *eventText = @{
                                         @"type" : @"TextBlock",
    ...
    // find yatzy grid features
    if (canSubmitForYatzyGridDetection) {
      _finishedDetectingYatzyGrid = false;
      self.startYatzyGrid = [NSDate date];
      [self.yatzyGridDetector
          findYatzyGridInFrame:image
                        scaleX:scaleX
                        scaleY:scaleY
                     completed:^(NSString *base64Image, NSArray *yatzyGrid,
                                 NSDictionary *error) {
                       NSDictionary *eventText = @{
                         @"error" : error,
    ...
    if (canSubmitForFaceDetection) {
      ...
      self.startFace = [NSDate date];
      [self.faceDetector findFacesInFrame:image
                                   scaleX:scaleX
                                   scaleY:scaleY
                                completed:^(NSArray *faces) {
                                  NSDictionary *eventFace =
                                      @{@"type" : @"face", @"faces" : faces};
    ...
    if (canSubmitForBarcodeDetection) {
      ...
      [self.barcodeDetector
          findBarcodesInFrame:image
                       scaleX:scaleX
                       scaleY:scaleY
                    completed:^(NSArray *barcodes) {
                      NSDictionary *eventBarcode =
                          @{@"type" : @"barcode", @"barcodes" : barcodes};
