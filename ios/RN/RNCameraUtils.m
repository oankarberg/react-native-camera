...
+ (NSString *)captureSessionPresetForVideoResolution:
    (RNCameraVideoResolution)resolution {
  switch (resolution) {
...
+ (UIImage *)convertBufferToUIImage:(CMSampleBufferRef)sampleBuffer
  ...
  if (curOrientation == UIInterfaceOrientationLandscapeLeft) {
    orientationToApply = isBackCamera ? kCGImagePropertyOrientationDown
                                      : kCGImagePropertyOrientationUpMirrored;
  } else if (curOrientation == UIInterfaceOrientationLandscapeRight) {
    orientationToApply = isBackCamera ? kCGImagePropertyOrientationUp
                                      : kCGImagePropertyOrientationDownMirrored;
  } else if (curOrientation == UIInterfaceOrientationPortrait) {
    orientationToApply = isBackCamera ? kCGImagePropertyOrientationRight
  ...
  float scale = scale =
      bufferHeight > bufferWidth ? 720 / bufferWidth : 720 / bufferHeight;
  ;
  // Was 400
  int maxSize = 750;
  if (position == 1) {
    scale = bufferHeight > bufferWidth ? maxSize / bufferWidth
                                       : maxSize / bufferHeight;
  }
  ...
  CGRect croppedSize =
      AVMakeRectWithAspectRatioInsideRect(previewSize, boundingRect);
  CGImageRef croppedCGImage =
      CGImageCreateWithImageInRect(videoImage, croppedSize);
